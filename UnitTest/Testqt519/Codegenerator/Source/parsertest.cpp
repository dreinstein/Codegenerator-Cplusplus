...
{
    std::vector<QString> values = parserText->doParseForVec(Path + "Files\\Scripts\\ClassTestClassMixedFunctionsAndAttributes.txt");
...
{
    std::vector<QString> values = parserXML->doParseForVec(Path + "Files\\Scripts\\xml\\MixedFunctionsAndAttributes.xml");
    EvaluateTest::evaluateParserVector(values,refValues);
}
...
{
    std::map<QString, QString> value = parserText->doParseForMap(Path + "Files\\Rules");
    QString cwd = Path +  "Files\\Rules\\class.txt";
...
{
    ASSERT_THROW( parserXML->doParseForVec(Path + "Files\\Scripts\\xml\\MixedFunctions.xml"),Errorhandling::OpenFileException);
}


TEST_F(ParserTest, ThrowExceptionCanNotOpenFileText)
{
    ASSERT_THROW( parserText->doParseForVec(Path + "Files\\Scripts\\xml\\MixedFunctions.txt"),Errorhandling::OpenFileException);
}


TEST_F(ParserTest, ThrowExceptionFileNotValidXML)
{
    ASSERT_THROW( parserXML->doParseForVec(Path + "Files\\Scripts\\xml\\FunctionWithNoParameterReturnIntNotValid.xml"),Errorhandling::XMLFileException);
}
